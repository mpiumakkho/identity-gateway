...
The verification workspace shows recent persisted sessions, allows a new session to be started, loads session detail by transaction ID, captures manual identity details for `MANUAL_ENTRY` sessions, accepts normalized Dip Chip payloads for `DIP_CHIP` sessions, and runs DOPA validation after identity data is captured.
